...
//Allumette Jeu::allumette=0;
void Jeu::init(){
	int nbAl=21;
	int maxi=3;
	this->allumette.allumette(nbAl,maxi);

}

void Jeu::setAllumettes(int nbAl){
	this->allumette.allumette(nbAl,3);
}
...
void Jeu::jouer(int mode){
	...
	while(this->allumette.getnb()>0) {
		...
		if (machine.getActuel()){
			...
			if (win()) {
				humain1.gagner();
				cout<<humain1.getNom()<<"a gagné!"<<endl;
						}
						else{
							cout<<endl;
		...
		while(this->allumette.getnb()>0) {
				...
				if (humain2.getActuel()){
					...
					if (win()) {
						humain2.gagner();
						cout<<humain2.getNom()<<" a gagné!"<<endl;
					}
					else{
						cout<<"Il reste "<<this->allumette.getnb()<<" allumettes"<<endl;
